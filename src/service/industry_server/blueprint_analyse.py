...
class BpAnalyser():
    ...
    def calculate_work_bpnode_quantity(self, typeid: int, cache_dict: dict):
        ...
        for edge in need_cal_edge:
            ...
            # 定义材料效率占位符, 从industry_config获取，输入为source_id, total_runs_needed, location_id
            coefficient = 1
            ...
            # TODO 此处调用工业配置获得一个流程数列，流程数列会参考现有蓝图。输入total_runs_needed, source_id
    ...
    def get_product_ori_materials(self, product: str, quantity: int = 1) -> dict:
        if not BPManager.check_product_id_existence(SdeUtils.get_id_by_name(product)):
